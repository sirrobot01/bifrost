...
func (r Runner) Run(ctx context.Context, arguments []string) int {
	...
	case "publish":
		err = r.runPublish(ctx, arguments[1:])
	case "upgrade":
...
func (r Runner) usage() {
	_, _ = fmt.Fprintln(r.Stderr, "usage: bifrost <doctor|init|check|publish|edge|serve|status|upgrade|version> [flags]")
	...
	_, _ = fmt.Fprintln(r.Stderr, "  publish  add a service to the configuration and reload")
