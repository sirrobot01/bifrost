...
Bifrost publishes self-hosted TCP services through native IPv6. It is for Linux and macOS hosts that have public IPv6 and use CGNAT for IPv4.
...
The home role must run on the host that owns the managed service addresses. Bifrost does not configure another host interface or an ISP router. Linux supports advisory and managed nftables modes; macOS uses advisory firewall mode.

## Quick start

Install the latest release. The script verifies the checksum and installs a native package or archive for the detected operating system. Linux packages create the `bifrost` account and systemd units; macOS ships a launchd definition in the archive. It starts nothing.
